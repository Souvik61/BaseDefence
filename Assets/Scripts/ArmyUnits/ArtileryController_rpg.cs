...
/// <summary>
/// RPG artillery controller
/// </summary>
public class ArtileryController_rpg : ArtileryController_t1
{
    ...
    public override IEnumerator ShootRoutine()
    {
        ...
        for (int i = 0; i < 2; i++)//for each muzzle
        {
            ...
            GameObject proj = Instantiate(commonAsset.ProjectilePrefab1_RPG, firePoints[i].position, Quaternion.identity);
            proj.GetComponent<BulletScript>().damageAmmount = (int)(10 * selfProperties.sDamage);
            proj.transform.rotation = Quaternion.LookRotation(Vector3.forward, firePoints[i].up);
